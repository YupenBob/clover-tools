---
title: "Base64 编码的坑：5 个翻车案例与修复方案"
description: "5个Base64翻车案例：Unicode中文乱码、填充丢失、URL-safe变种、Data URI大小膨胀、btoa限制，附JS/Python修复方案。"
tags: ["Base64", "编码", "前端"]
category: "编码加密"
original: true
planned_date: "2026-08-20"
link: "https://clovertools.cn/tools/dev/base64/"
status: draft
---

Base64 看着简单，实际用起来坑不少：中文编码错乱、填充符号被丢、URL 里 + 和 / 被转义。这些坑大多不是 Base64 本身的问题，而是编码链路两端没对齐。这篇把几个典型场景和修复方法记下来。
...
Base64 本质上是个**编码系统**，不是加密。它用 64 个字符（A-Z、a-z、0-9、+、/）来表示二进制数据。三个字节转成四个字符，不够就补 `=` 填充。
...
Base64 本身不复杂，大多数翻车都是因为**编码链路的某一端没有对齐**：你用 UTF-8 发，后端用 GBK 接；你去掉填充发，后端没补回来。找坑的时候顺着链路查，哪个环节用的是 ASCII 字符集，哪个环节就可能是罪魁祸首。

如果你想直接动手试试这几个坑的修复方案，我给你准备了一个在线工具集：

 ****  支持 UTF-8 中文、URL-safe 模式、填充检查，直接用。

有问题直接去工具页测试，比对着代码查快多了。

---

踩 Base64 的坑时，先确认两端字符集一致，再检查填充和 URL-safe 模式。想快速验证的话，把字符串丢进[在线 Base64 编解码工具](https://clovertools.cn/tools/dev/base64/)，切一下 UTF-8 和 URL-safe 选项就能看到差异，比对着代码猜快。
