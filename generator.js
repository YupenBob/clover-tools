...
const TOOL_TYPE_REGISTRY = {
  ...
  // type: "encoder" → input + key input + encrypt/decrypt buttons + output (for bcrypt, des, rc4, rabbit)
  'encoder': {
    description: '加密解密工具',
    html: function(tool) {
      return '<div class="tool-card"><h3>输入</h3><textarea id="input" placeholder="输入要加密或解密的文本..." style="min-height:100px;"></textarea><div class="input-field"><label>密钥(Key)</label><input type="text" id="key" placeholder="输入加密密钥" style="width:100%;padding:0.5rem;font-size:1rem;"></div><div class="btn-row"><button class="btn btn-primary" id="encryptBtn">加密</button><button class="btn btn-secondary" id="decryptBtn">解密</button></div></div><div class="output-box"><h3>输出 <button class="copy-btn" id="copyOutput">复制</button></h3><textarea id="output" readonly style="min-height:100px;"></textarea></div>';
    },
    script: function(tool) {
      return "var CryptoJS=null;var bcryptjs=null;var loadedCryptoJS=false;var loadedBcrypt=false;function checkReady(){}function loadCryptoJS(){if(loadedCryptoJS)return;loadedCryptoJS=true;var s=document.createElement('script');s.src='https://cdnjs.cloudflare.com/ajax/libs/crypto-js/4.2.0/crypto-js.min.js';s.onload=function(){CryptoJS=window.CryptoJS;checkReady();};s.onerror=function(){document.getElementById('output').value='CryptoJS 加载失败,请检查网络';};document.head.appendChild(s);}function loadBcrypt(){if(loadedBcrypt)return;loadedBcrypt=true;var s=document.createElement('script');s.src='https://cdnjs.cloudflare.com/ajax/libs/bcryptjs/2.4.3/bcrypt.min.js';s.onload=function(){bcryptjs=window.dcodeIO&&window.dcodeIO.bcrypt;checkReady();};s.onerror=function(){document.getElementById('output').value='bcrypt.js 加载失败,请检查网络';};document.head.appendChild(s);}function escHtml(s){return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}document.getElementById('encryptBtn').onclick=function(){var t=document.getElementById('input').value;var k=document.getElementById('key').value;if(!t){document.getElementById('output').value='请输入文本';return;}var p=tool.path||'';try{if(p.includes('bcrypt')){if(!bcryptjs){document.getElementById('output').value='bcrypt 库加载中...';loadBcrypt();return;}var hash=dcodeIO.bcrypt.hashSync(t,10);document.getElementById('output').value=hash;}else if(p.includes('des')){if(!CryptoJS){loadCryptoJS();document.getElementById('output').value='CryptoJS 加载中...';return;}if(!k){document.getElementById('output').value='请输入密钥';return;}document.getElementById('output').value=CryptoJS.DES.encrypt(t,k).toString();}else if(p.includes('rc4')){if(!CryptoJS){loadCryptoJS();document.getElementById('output').value='CryptoJS 加载中...';return;}if(!k){document.getElementById('output').value='请输入密钥';return;}document.getElementById('output').value=CryptoJS.RC4.encrypt(t,k).toString();}else if(p.includes('rabbit')){if(!CryptoJS){loadCryptoJS();document.getElementById('output').value='CryptoJS 加载中...';return;}if(!k){document.getElementById('output').value='请输入密钥';return;}document.getElementById('output').value=CryptoJS.Rabbit.encrypt(t,k).toString();}else{document.getElementById('output').value='未知加密类型';}}catch(e){document.getElementById('output').value='加密错误: '+escHtml(e.message);}};document.getElementById('decryptBtn').onclick=function(){var c=document.getElementById('input').value;var k=document.getElementById('key').value;if(!c){document.getElementById('output').value='请输入密文';return;}var p=tool.path||'';try{if(p.includes('bcrypt')){if(!bcryptjs){document.getElementById('output').value='bcrypt 库加载中...';loadBcrypt();return;}var valid=dcodeIO.bcrypt.verify(c,k);document.getElementById('output').value=valid?'验证成功: 密码匹配':'验证失败: 密码不匹配';}else if(p.includes('des')){if(!CryptoJS){loadCryptoJS();document.getElementById('output').value='CryptoJS 加载中...';return;}if(!k){document.getElementById('output').value='请输入密钥';return;}var d=CryptoJS.DES.decrypt(c,k);document.getElementById('output').value=d.toString(CryptoJS.enc.Utf8)||'解密失败';}else if(p.includes('rc4')){if(!CryptoJS){loadCryptoJS();document.getElementById('output').value='CryptoJS 加载中...';return;}if(!k){document.getElementById('output').value='请输入密钥';return;}var d=CryptoJS.RC4.decrypt(c,k);document.getElementById('output').value=d.toString(CryptoJS.enc.Utf8)||'解密失败';}else if(p.includes('rabbit')){if(!CryptoJS){loadCryptoJS();document.getElementById('output').value='CryptoJS 加载中...';return;}if(!k){document.getElementById('output').value='请输入密钥';return;}var d=CryptoJS.Rabbit.decrypt(c,k);document.getElementById('output').value=d.toString(CryptoJS.enc.Utf8)||'解密失败';}else{document.getElementById('output').value='未知加密类型';}}catch(e){document.getElementById('output').value='解密错误: '+escHtml(e.message);}};document.getElementById('copyOutput').onclick=function(){navigator.clipboard.writeText(document.getElementById('output').value);};";
    }
  },

  // type: "diff" → input text + regex pattern + replacement input + replace button → output
  'diff': {
    description: '正则替换工具',
    html: function(tool) {
      return '<div class="tool-card"><h3>输入文本</h3><textarea id="input" placeholder="输入要替换的文本..." style="min-height:100px;"></textarea></div><div class="tool-card"><h3>正则表达式</h3><input type="text" id="regexInput" placeholder="如 \\d+ 表示数字" style="width:100%;padding:0.5rem;font-size:1rem;margin-bottom:0.75rem;"><h3 style="margin-top:0.75rem;">替换为</h3><input type="text" id="replaceInput" placeholder="替换结果,留空则删除匹配" style="width:100%;padding:0.5rem;font-size:1rem;margin-bottom:0.75rem;"><div class="btn-row"><button class="btn btn-primary" id="replaceBtn">执行替换</button><button class="btn btn-secondary" id="clearBtn">清空</button></div></div><div class="output-box"><h3>结果 <button class="copy-btn" id="copyOutput">复制</button></h3><textarea id="output" readonly style="min-height:100px;"></textarea></div>';
    },
    script: function(tool) {
      return "document.getElementById('replaceBtn').onclick=function(){var input=document.getElementById('input').value;var pattern=document.getElementById('regexInput').value;var replacement=document.getElementById('replaceInput').value;if(!pattern){document.getElementById('output').value='请输入正则表达式';return;}try{var regex=new RegExp(pattern,'g');document.getElementById('output').value=input.replace(regex,replacement);}catch(e){document.getElementById('output').value='正则表达式错误: '+e.message;}};document.getElementById('clearBtn').onclick=function(){document.getElementById('input').value='';document.getElementById('regexInput').value='';document.getElementById('replaceInput').value='';document.getElementById('output').value='';};document.getElementById('copyOutput').onclick=function(){navigator.clipboard.writeText(document.getElementById('output').value);};";
    }
  }
};
...
const TOOL_SPECIFIC_OVERRIDES = {
  ...
  'code/cron-parser.html': {
    ...
    'encrypt/aes.html': {
    html: '<div class=\"tool-card\"><h3>输入</h3><textarea id=\"input\" placeholder=\"输入要加密或解密的文本...\" style=\"min-height:100px;\"></textarea><div class=\"input-field\"><label>密钥(Key)</label><input type=\"text\" id=\"key\" placeholder=\"输入加密密钥\" style=\"width:100%;padding:0.5rem;font-size:1rem;\"></div><div class=\"btn-row\"><button class=\"btn btn-primary\" id=\"encryptBtn\">加密</button><button class=\"btn btn-secondary\" id=\"decryptBtn\">解密</button></div></div><div class=\"output-box\"><h3>输出 <button class=\"copy-btn\" id=\"copyOutput\">复制</button></h3><textarea id=\"output\" readonly style=\"min-height:100px;\"></textarea></div>',
    script: 'var CryptoJS=null;var s=document.createElement(\"script\");s.src=\"https://cdnjs.cloudflare.com/ajax/libs/crypto-js/4.2.0/crypto-js.min.js\";s.onload=function(){CryptoJS=window.CryptoJS;};document.head.appendChild(s);document.getElementById(\"encryptBtn\").onclick=function(){if(!CryptoJS){document.getElementById(\"output\").value=\"库加载中...\";return;}var t=document.getElementById(\"input\").value;var k=document.getElementById(\"key\").value;if(!t||!k){document.getElementById(\"output\").value=\"请输入文本和密钥\";return;}document.getElementById(\"output\").value=CryptoJS.AES.encrypt(t,k).toString();};document.getElementById(\"decryptBtn\").onclick=function(){if(!CryptoJS){document.getElementById(\"output\").value=\"库加载中...\";return;}var c=document.getElementById(\"input\").value;var k=document.getElementById(\"key\").value;try{document.getElementById(\"output\").value=CryptoJS.AES.decrypt(c,k).toString(CryptoJS.enc.Utf8);}catch(e){document.getElementById(\"output\").value=\"解密失败，请检查密钥是否正确\";}};document.getElementById(\"copyOutput\").onclick=function(){navigator.clipboard.writeText(document.getElementById(\"output\").value);};'
  },
