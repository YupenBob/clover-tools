...
function generate() {
  ...
  toolsConfig.forEach(cat => {
    cat.tools.forEach(tool => {
      urls.push(`<url><loc>${baseUrl}/tools/${tool.path}</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>`);
    });
  });
  // Add blog posts to sitemap
  keywordsConfig.forEach(kw => {
    urls.push(`<url><loc>${baseUrl}/blog/${kw.slug}</loc><changefreq>weekly</changefreq><priority>0.7</priority></url>`);
  });
  urls.push(`<url><loc>${baseUrl}/blog/</loc><changefreq>daily</changefreq><priority>0.7</priority></url>`);

  // Add fix hub pages to sitemap
  FIX_HUB_CONFIG.forEach(hub => {
    urls.push(`<url><loc>${baseUrl}/fix/${hub.path}/</loc><changefreq>weekly</changefreq><priority>0.6</priority></url>`);
  });
